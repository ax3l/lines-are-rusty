...
pub fn parse_line(cursor: &mut io::Cursor<&[u8]>) -> Result<Line, io::Error> {
    ...
    let color = cursor.read_i32::<Endian>()?;
...
pub fn read_layers(
    cursor: &mut io::Cursor<&[u8]>,
    _max_size_file: usize,
) -> Result<Vec<Layer>, io::Error> {
    println!("Parsing layers");
    let mut layers = vec![];
    ...
    for _l in 0..read_number_i32(&num_layers) {
        ...
        let lines = read_lines(cursor, _max_size_file)?;
        layers.push(Layer { lines });
    }
    Ok(layers)
}

pub fn read_page(content: &[u8], _max_size_file: usize) -> Result<Page, io::Error> {
    println!("Parsing v5 page");
